...
- Raw files in the cerebro layout carry `creationDate: YYYY-MM-DD` (local date) next to `fecha`, so indexers that date documents by `creationDate` (System 3's `--since/--until`) see mail, calendar and meeting files.
